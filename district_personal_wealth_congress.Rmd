...
vars <- paste0("B19326_001", "B19013_001", "")#median income over past 12 months, median household income, poverty
...
`108`$quintile <- as.factor(ntile(`108`$assets_mean, 5))
`109`$quintile <- as.factor(ntile(`109`$assets_mean, 5))
`110`$quintile <- as.factor(ntile(`110`$assets_mean, 5))
`111`$quintile <- as.factor(ntile(`111`$assets_mean, 5))
`112`$quintile <- as.factor(ntile(`112`$assets_mean, 5))
`113`$quintile <- as.factor(ntile(`113`$assets_mean, 5))
...
#add in variables 
plot(cel_acs_os$quintile, cel_acs_os$cd_median_income)
...
summary(lm(log(cd_median_income) ~ q1 + q5 + seniority + factor(congress) + deleg_size  + female, cel_acs_os)) #wealthiest members come from slightly wealthier areas q5 has almost 2% higher median income nationwide, and 4% higher median income within states. There's no difference for q1.  
...
#shows that 1% increase in cd median income is associated with a 14.5% increase in probability in a q5 rep. Holds most within states.
...
summary(lm(les ~ q1 + q5 + log(cd_median_income)  + seniority + sensq + state_leg + state_leg_prof + majority + maj_leader + min_leader + maj_leader + chair + subchr + power + meddist + female + afam + latino  + votepct  +  votepct_sq + deleg_size, cel_acs_os)) #one unit increase in cd median income associated with .33 increase in les.
